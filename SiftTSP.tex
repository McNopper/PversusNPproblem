...
\paragraph{Termination.} Inside a fixed $d_{\max}$, the inner
\texttt{while} loop runs at most $\lfloor \log_2 m_{\text{ceiling}}
\rfloor + \lfloor \log_2 s_{\text{ceiling}} \rfloor + 8$ sweeps: $m$
doubles at most $\lfloor \log_2 m_{\text{ceiling}} \rfloor$ times
before saturating and likewise for $s$, after which the loop must hit
8 consecutive non-improving sweeps and break. The outer \texttt{for}
loop runs over $d_{\max} = 1, \dots, d_{\text{ceiling}}$. Hence the
total number of sweeps is finite. The running best is updated only when
strictly improved, so the returned tour has length monotone
non-increasing in the execution trace and is bounded below by $0$.
