...
**[Author Name]** — *[Institution], [City, Country]*
...
For any fixed parameters the algorithm runs in $O(n \log n)$ time when
$\tau \cdot 2^{d_{\max}} \geq n$ (all-exact regime) and in $O(n^2)$
otherwise. SiftTSP is **not exact**: we exhibit an explicit 12-point
...
*decomposition irrecoverability* (Proposition 2): no fixed geometric
decomposition can recover an optimal tour that interleaves cities across
the decomposition's components. Consequently this work makes **no claim**
